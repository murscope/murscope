...
REWRITES = {
    ...
    "scripts/checks/all_changes_via_pr.py": (
        (
            "def main():\n    refs = main_refs()\n",
            "def main():\n"
            "    # **Rule 2 is about work landing on `main`, and nothing "
            "lands in a\n"
            "    # derivation.** What arrives here is extraction output, and "
            "the review\n"
            "    # this rule exists to guarantee happened in the repository "
            "this tree\n"
            "    # was derived from, before the commit that was extracted. So "
            "the mode\n"
            "    # is named and the assertion is not made - and it is not "
            "skipped,\n"
            "    # because a skipped check and a passed one are the same "
            "green in a\n"
            "    # summary, which is a distinction this line settled long "
            "ago.\n"
            "    #\n"
            "    # **What that costs is stated rather than left to be "
            "found.** Between\n"
            "    # the founding commit and this branch, this check was the "
            "only thing\n"
            "    # standing between this `main` and a hand edit pushed "
            "straight to it -\n"
            "    # accidentally, and only from the second commit onward. "
            "Nothing here\n"
            "    # measures the claim PUBLICATION.md makes, that everything "
            "in this\n"
            "    # tree arrived by derivation. The property that should hold "
            "is that\n"
            "    # every commit is an extraction output, and **this tree "
            "cannot check\n"
            "    # that about itself**: the evidence is in a repository it "
            "does not\n"
            ...
            "        if not REFERENCE.search(carrying) or "
            "REFERENCE.search(bare):\n"
            ...
            "        return 0\n"
            "    refs = main_refs()\n",
            "Rule 2 asks that work reach `main` through a pull request, and "
            "in a derivation no work reaches `main` at all - extraction "
            "output does, and the review the rule exists to guarantee "
            "happened in the source repository before the commit that was "
            "extracted. Published unchanged, the rule made the second "
            "derivation push red and offered only one way out: a pull "
            "request in the public repository, opened and merged by the same "
            "person. **That is the shape this rule's own history warns "
            "about** - its first enforcement was a checkbox in the pull "
            "request template, ticked on every pull request and wrong the "
            "whole time, so buying a green square with a review nobody "
            "reviewed is the one outcome to refuse here. The published form "
            "states the mode, states what it therefore does not assert, and "
            "**states the loss in the same breath**: nothing anywhere now "
            "measures PUBLICATION.md's claim that every commit here is an "
            "extraction output, and this tree cannot measure it, because the "
            "evidence lives in a repository it does not have. It proves its "
            "matcher either way rather than skipping, because a skip and a "
            "pass are the same green in a summary",
        ),
        (
...
REGEX_REWRITES = {
    ...
    "scripts/checks/all_changes_via_pr.py": (
        (
            ...
            "repository, and publishing it would put a coordinate into "
            "a history nobody can open - Rule 35's coordinate half "
            "reports exactly that, and does so on this line if the "
            "rewrite is removed, which is how this reason was "
            "re-established rather than remembered. **It used to say "
            "something else, and that something else has expired.** The "
            "reason given here was that a tree founded clean holds no "
            "such object, so the check would be red on its first run "
            "there. That was true while the published check still read "
            "this constant; it no longer does - the derivation branch "
            "above returns before reaching it. The disclosure reason "
            "stands on its own and is the whole of why this entry is "
            "still here (DP174)",
        ),
        (
            ...
            "in the published tree - a coordinate into a history the "
            "reader cannot open, which is the whole of why this entry "
            "exists. The second half of what was written here has "
            "expired with the entry above it: it said every one of them "
            "would be reported there as a stale exception, and the "
            "published check now returns before it could report "
            "anything of the sort (DP174)",
        ),
    ),
}
